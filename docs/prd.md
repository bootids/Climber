...
### 0.1 世界观公理 (World Axioms)
  ...
  * **时间堆积 (Time Sedimentation)**： 物理时间不再向前流动，而是像沉积岩一样向上堆叠。海拔越高，年代越古老。
    ...
    * `8000m = 远古时代` (非欧几何带)
  * **核心隐喻**： "攀登即考古，高处即过去。"
  * **玩家身份**： **记忆修补匠 (Mnemo-Fixer)**。隶属于"档案馆"，任务是回收位于不同历史断层的"关键锚点"以揭开大停滞的真相。
  * **黑色方碑 (Black Box)**： 前任修补匠的尸体/黑匣子，既是路标也是叙事碎片。包含死者的记忆片段和关键信息。

### 0.2 核心机制术语

  * **动能势能 (Kinetic Potential, KP)**： 游戏内的核心货币，不再限制移动。仅用于购买补给、升级装备、呼叫救援。通过每日签到（卫星连线）、节点结算、探索发现获取。
  * **体力 (Stamina)**： 移动的直接燃料。移动时直接消耗体力，消耗速率受地形阻力系数 (Drag Coefficient) 影响（如碎石坡、深雪）。通过消耗 KP 购买食物/休息来恢复。
  * **线性矢量微调滑尺 (Linear Vector Trim Dial, VTD)**： 玩家用于手动设定横向偏移量的线性水平滑尺 UI 控件。带刻度的标尺（-20m ... 0m ... +20m），中间有指针。屏幕滑动距离与游戏内偏移量 1:1 对应，提供精密仪器感。用于抵消环境风力、主动设定偏移量以绕过障碍，或进行**相位调谐**（调频同步历史信号）。
  ...
  * **幽灵频段 (Ghost Frequency)**： VTD 仪表上的幻影光标。代表 NPC 当年的移动轨迹。玩家需手动操控滑尺与幽灵光标重叠，保持重叠时体力消耗减半（尾流效应），且日志栏会刷新特定颜色的 NPC 剧情文本。
  * **相位调谐 (Phase Tuning)**： 主动挖掘机制。在特定地点驻足，将 VTD 变为"收音机旋钮"，左右滑动寻找清晰的历史音频信号（文本回放）。
  * **视差解谜 (Parallax Puzzle)**： 利用 SAN 值系统。低 SAN 值（幻觉状态）下才能看到某些被隐藏的历史路径（如穿过全息伪装的墙）。
  * **矢量解耦 (Vector Decoupling)**： "迷路"的程序实现方式。此时 VTD 仪表欺诈：UI 显示在 0m（主路），但实际坐标已偏移。真相在文字里：滚动日志会提示与 UI 矛盾的信息（如"风向不对"、"有树枝刮擦"）。玩家需根据文字线索逻辑推理出偏差值，手动反向拖动滑尺进行盲操作补偿。
...
**设计理念**： 游戏明确为纯 UI + 文本驱动（无 3D 角色渲染），所有动态机制均进行符号化抽象。

1.  **长按推进 (Press & Hold)**： 玩家按住屏幕底部的推进区驱动角色前进。按住 = 消耗体力 + 推进里程。松手 = 驻足（暂停消耗，用于思考/检查）。通过音效与震动反馈模拟地形阻力（如深雪时推进速度变慢，震动沉闷）。
2.  **文本监视 (The Vigilant Walker)**： 行进中，玩家必须盯着屏幕上方不断刷新的滚动日志。从文字中捕捉危险信号（如"冰裂声"、"脚感变软"、"碾压判定：致命"），并在危机发生前主动松手做出决策。
3.  **红区压迫 (The Red Noise)**： 地图底部出现的红色噪点/马赛克，像进度条一样向上吞噬 UI。滚动日志被乱码和红色警告污染。玩家必须在"长按推进逃命（消耗大量体力）"和"精准卡在红区边缘休息（赌命）"之间博弈。
4.  **线性VTD操控**： 左右拖动线性滑尺来抵消环境风力、主动设定偏移量以绕过障碍，或进行**相位调谐**（调频同步历史信号）。屏幕滑动距离与游戏内偏移量 1:1 对应。
5.  **幽灵频段 (Ghost Frequency)**： VTD 滑尺上出现一个半透明的、自动移动的"幽灵光标"（代表 NPC 当年的移动轨迹）。玩家需手动操控滑尺与幽灵光标重叠。保持重叠时，体力消耗减半（尾流效应），且日志栏会刷新特定颜色的 NPC 剧情文本。
6.  **相位调谐 (Phase Tuning)**： 在特定地点驻足，将 VTD 变为"收音机旋钮"，左右滑动寻找清晰的历史音频信号（文本回放）。
7.  **双轨制资源管理**： 体力 = 燃料（移动直接消耗）；KP = 货币（购买补给、升级装备、呼叫救援）。体力通过消耗 KP 购买食物/休息来恢复。
...
- [飘雪] (Snow):
  ...
  * **UI 易用性**： 确保所有大气特效（如飘雪堆积）都位于 UI 交互层的下方，或保证 100% 的点击穿透。"晃动手机抖落积雪"保留为纯视觉互动，但不阻碍操作。

- [暴雨] (Rain):
  * **特效**: 屏幕上有雨水滑落的扭曲折射效果 (Refraction)。
  * **滤镜**: 整体色调偏蓝冷，伴随随机的白屏闪烁（雷电）。

### 2.3 时间潮汐滤镜 (Time Tides Grading)
...
### 3.3 线性矢量微调滑尺 (Linear VTD) 
  ...
  * **交互逻辑**：
    * **线性对应**：屏幕滑动距离与游戏内偏移量 1:1 对应，提供精密仪器感。
    * **用途**：左右拖动滑尺来抵消环境风力，或主动设定偏移量以绕过障碍。
    * **手感反馈**：拖动时提供轻微震动和音效反馈，模拟精密仪器的操作感。
...
- **消耗用途**：
  ...
  * [恢复体力]：购买食物/休息来恢复体力
  * [技能学习]：购买技能经验书
- **存储上限**：初始上限 20,000 KP，可通过商业化升级提升。
...
- **长按推进**：简单的按住操作，无需复杂手势。
- **线性VTD**：直观的滑动操作，1:1 对应关系。
- **松手驻足**：随时可以暂停思考，降低心理压力。
...
- **阶段一**：长按推进与体力系统：
  * 引导玩家按住屏幕底部的推进区，体验长按推进机制。
  * 演示体力消耗和地形阻力反馈（音效、震动）。
  * 演示松手驻足功能（暂停消耗，用于思考）。
  * 介绍 KP 获取途径（每日签到、节点结算等）。

- **阶段二**：文本监视与VTD操控：
  * 引导玩家观察屏幕上方滚动日志，识别危险信号。
  * 演示线性 VTD 滑尺操作（左右拖动调整偏移）。
  * 强制触发迷路状态（VTD 仪表欺诈）。
...
## 13. 商业化策略 (Monetization Strategy)
...
- **电池扩容 (Battery Upgrade)**：
  ...
  * **付费解锁**：50k / 100k 上限（方便玩家存储更多 KP，用于购买高级装备和补给）。
...
### 13.3 激励广告 (Ads)
  ...
  * **紧急除颤**： 死亡时，可观看广告获得一次"原地复苏"机会（仅回复 10% 状态，且带有长时间冷却，无法连续使用）。对广告复活施加严格限制，以保持 Roguelike 的死亡重量感。例如，每个赛季限制使用次数，或复活后携带一个永久性的 Debuff。
